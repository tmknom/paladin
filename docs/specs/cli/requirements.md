...
- 解析対象の `.py` ファイルを診断し、違反レポートを標準出力へ表示できる
- `--rule <rule-id>` オプションで適用ルールを限定できる（複数回指定可）
- `--ignore-rule <rule-id>` オプションで特定ルールの違反をグローバルに無視できる（複数回指定可）
- `--format <format>` オプションで出力形式を指定できる（`text` / `json`）

### list サブコマンド

- 利用可能なルールの一覧をテキスト形式で標準出力へ表示できる

### view サブコマンド

- 指定したルールの詳細情報をテキスト形式で標準出力へ表示できる
- ルール ID を positional argument として受け取る
- 指定した rule_id が存在しない場合はエラーメッセージを表示できる（exit code 0）

### version サブコマンド

- パッケージのバージョン文字列を標準出力へ表示できる
