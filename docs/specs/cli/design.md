...
| `check` | `@app.command()` 関数 | check サブコマンドの実行（`--format` オプションで出力形式を指定） |
| `list_rules` | `@app.command(name="list")` 関数 | list サブコマンドの実行 |
| `view` | `@app.command()` 関数 | view サブコマンドの実行 |
| `version` | `@app.command()` 関数 | version サブコマンドの実行 |
