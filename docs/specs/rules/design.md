...
**トレードオフ**: rules モジュールが `check` モジュールに依存する形になる。将来的にルール定義を独立したパッケージへ移動する場合は、両モジュールの変更が必要になる。なお、`check` モジュール設計書にも記載されているとおり、ルール定義の配置場所は `rules` コマンドの設計確定時に再検討するトリガーがある。
...
- **ルール定義の独立化**: `check` モジュール依存を解消するために、ルール定義を専用パッケージへ移動する（`rules` コマンドの設計確定時に判断する）
