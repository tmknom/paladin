...
- **check モジュールへの依存**: ルール定義とレジストリは `check` モジュールに置き、rules モジュールは参照するだけにとどめる
...
| 依存先 | 用途 |
|---|---|
| `paladin.check.rule.registry.RuleRegistry` | 登録済みルールのメタ情報一覧の管理 |
| `paladin.check.rule.protocol.Rule` | ルール実装が満たすべき契約の参照 |
| `paladin.check.rule.types.RuleMeta` | ルールメタ情報の型定義 |
...
| オーケストレーター | `RulesOrchestrator` | ルール一覧取得とフォーマットの処理フロー制御 |
| フォーマッター | `RulesFormatter` | `tuple[RuleMeta, ...]` をテキスト形式の文字列に変換 |

### 処理フロー概略

1. `RulesOrchestratorProvider.provide()` が `RuleRegistry` と `RulesFormatter` を組み立てて `RulesOrchestrator` を返す
2. CLI が `RulesOrchestrator.orchestrate()` を呼び出す
3. `RuleRegistry.list_rules()` が登録済みルールの `RuleMeta` 一覧を返す
4. `RulesFormatter.format()` が `RuleMeta` 一覧を列幅を揃えたテキスト形式に変換して返す
...
### check モジュールのルール定義を再利用する

**設計の意図**: ルール定義（`Rule` Protocol・`RuleRegistry`・`RuleMeta` 等）は `check` モジュールに置き、rules モジュールはそれらを参照するだけにとどめる。

**なぜそう設計したか**: `check` コマンドが適用するルール群と `rules` コマンドが一覧表示するルール群は同一である。重複管理を避けるために、ルール定義の単一の情報源として `check` モジュールを利用する。

**トレードオフ**: rules モジュールが `check` モジュールに依存する形になる。将来的にルール定義を独立したパッケージへ移動する場合は、両モジュールの変更が必要になる。なお、`check` モジュール設計書にも記載されているとおり、ルール定義の配置場所は `rules` / `explain` コマンドの設計確定時に再検討するトリガーがある。

### Context を持たない設計

**設計の意図**: `rules` コマンドは実行時パラメータを必要としないため、`RulesOrchestrator.orchestrate()` は引数なしで呼び出せる設計とする。

**なぜそう設計したか**: `check` コマンドの `CheckContext` のように CLI 引数や実行時パラメータを渡す必要がなく、登録済みルール一覧は静的に決まる。不要な抽象化（YAGNI 原則）を避けるために Context を導入しない。

**トレードオフ**: 将来的に `--format` オプション等の引数が追加された場合、Context の導入または引数追加が必要になる。
...
ビジネスロジック層
  rules/
    __init__.py              # 公開 API（RulesOrchestrator / RulesOrchestratorProvider）
    provider.py              # RulesOrchestratorProvider（Composition Root）
    orchestrator.py          # RulesOrchestrator
    formatter.py             # RulesFormatter
      ↓ list_rules() / format()
check モジュール（再利用）
  check/rule/
...
RulesOrchestrator.orchestrate()
    ├─ RuleRegistry.list_rules() → tuple[RuleMeta, ...]
    └─ RulesFormatter.format(rules) → str
      ↓
CLI層
...
現在の公開 API: `RulesOrchestrator`, `RulesOrchestratorProvider`

### ルール登録の変更手順

rules モジュールが表示するルール一覧は、`RulesOrchestratorProvider._create_rules()` のタプルで管理されている。新しいルールを追加する場合は、以下の手順を踏むこと。

1. `check/rule/` サブパッケージに `Rule` Protocol を満たすクラスを実装する
2. `RulesOrchestratorProvider._create_rules()` のタプルに追加する
3. `check` モジュールの `CheckOrchestratorProvider._create_runner()` にも同様に追加する

`check` コマンドと `rules` コマンドの両方に影響するため、2箇所の変更が必要である。
...
- **ルール定義の独立化**: `check` モジュール依存を解消するために、ルール定義を専用パッケージへ移動する（`rules` / `explain` コマンドの設計確定時に判断する）
...
| 変更内容 | 主な変更対象 | 備考 |
|---|---|---|
| 新しいルールを追加 | `check/rule/` に新ファイル、`provider.py`（`_create_rules()`）、`check` の `CheckOrchestratorProvider` も同様に変更 | 2箇所の Provider 変更が必要 |
...
- [check モジュール基本設計](../check/design.md): ルール定義基盤の設計（`Rule` Protocol・`RuleRegistry`・ルール実装）
