...
4. `CheckResult`（`TargetFiles` + `ParsedFiles` + `Violations`）として返す

その後、CLI 層が `CheckReportFormatter.format(result)` を呼び出し、`CheckResult` を text 形式の `CheckReport` に変換します。
...
**設計の意図**: `CheckOrchestrator` は `CheckResult` を返すことのみに専念し、テキストフォーマットの責務は `CheckReportFormatter` に分離する。

**なぜそう設計したか**: フォーマット形式（text / JSON 等）は出力先によって異なりうる関心事であり、パイプライン処理とは変更理由が異なる。Orchestrator に含めると出力形式の変更がパイプライン処理の変更と混在し、影響範囲が広がる。`CheckReportFormatter` を別クラスとして CLI 層に公開することで、フォーマット変更を `formatter.py` のみに局所化できる。

**トレードオフ**: CLI 層が `CheckOrchestrator` と `CheckReportFormatter` の両方を扱う必要がある。ただし両者の依存は単方向であり、フロー全体の見通しは `cli.py` で確認できる。
...
現在の公開 API: `CheckContext`, `CheckOrchestratorProvider`, `CheckReport`, `CheckReportFormatter`, `CheckStatus`, `CheckSummary`, `RuleMeta`, `Violation`, `Violations`
...
| 呼び出し元 | ファイル | 影響する変更 |
|---|---|---|
| CLI の check コマンド | `src/paladin/cli.py` | `CheckContext` のフィールド追加・変更、`CheckOrchestratorProvider` のインターフェース変更、`CheckReportFormatter` のインターフェース変更 |
