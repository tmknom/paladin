...
現時点でルール個別設定を持つルールはありません。`require-qualified-third-party` の `root-packages` パラメータは廃止されました。ルートパッケージは解析対象ファイルのパスから自動的に導出されるため、手動設定は不要です。
...
この例では、`no-relative-import` だけを無効化しています。全ルールがデフォルトで有効なため、無効化したいルールのみ記述します。
...
実装順序としては R-090（ルール ON/OFF）→ R-092（ルール個別設定）→ R-091（include/exclude）→ R-093（overrides）が自然です。R-090 が設定ファイル読み込みの基盤を構築し、R-091 と R-093 はそれぞれ独立した機能であり、R-090 の基盤の上に構築します。なお、R-092 として導入した `require-qualified-third-party` の `root-packages` 設定は、ルートパッケージの自動導出（ファイルパスからの解析）に置き換えられ廃止されています。
