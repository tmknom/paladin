...
Fixture 設計の **分離性** をディレクトリ構成レベルで実現するためのポリシーです。
各テストメソッドは、独立したフィクスチャディレクトリを指定します。

- テストシナリオごとに専用のフィクスチャディレクトリを持つ
- 既存の `violation/` や `compliant/` に異なるシナリオのファイルを混在させない
- 混在させるとフィクスチャの意図が不明確になり、変更時の影響範囲が拡大する

基本パターンは `violation/` と `compliant/` の2ディレクトリ構成です。
同じカテゴリ（violation または compliant）で複数のシナリオが必要な場合は、`{カテゴリ}_{シナリオ名}` 形式の追加ディレクトリを作成します。
