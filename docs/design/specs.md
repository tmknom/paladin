...
### docs/specs/（変更頻度の高い Feature モジュール）
...
### docs/internal/（変更頻度の低い基盤モジュール）

基盤モジュール（error, fs, log, model, protocol, cli）の仕様ドキュメントは `docs/internal/` で管理する。
requirements.md と design.md を1ファイルに統合し、モジュールの性質に合わせた構成で記述する。

| ファイル | 対象モジュール |
|---------|--------------|
| `error.md` | `foundation/error` |
| `fs.md` | `foundation/fs` |
| `log.md` | `foundation/log` |
| `model.md` | `foundation/model` |
