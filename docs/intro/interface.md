...
本文書は、Paladin の実装済み CLI インターフェイスの仕様を記述します。
...
存在しないルール ID を指定した場合、エラーメッセージを出力して終了コード `0` で終了します。ルール未検出は実行失敗ではなく、正常終了として扱います。
...
- [Paladin 全体仕様](specifications.md): 本ツールが満たすべき要求事項の定義
