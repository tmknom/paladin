...
このルールは単一ファイル内のインポートパスのパターンで部分的に検出可能ですが、完全な検出には将来の複数ファイル解析が必要です。

- 完全な検出には「対象のシンボルが `__init__.py` で公開されているかどうか」の判定が必要であり、複数ファイルの解析を要する
- 初期実装では「`__init__.py` を経由しない深いパス（3階層以上）」といったヒューリスティックな検出でフォールバックする可能性がある
