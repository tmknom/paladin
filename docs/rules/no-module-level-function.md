...
@app.command()
def check(...) -> None:  # 準拠: allow-files に登録されたファイルはルール全体を除外
    ...
```
...
allow-files = ["src/paladin/cli.py", "src/paladin/foundation/log/decorator.py"]
```

- `allow-decorators`: 許可するデコレータ名のリスト。デフォルトは `["pytest.fixture", "fixture"]`
- `allow-files`: 検査を除外するファイルのリスト。デフォルトは空
    - 比較方式は **完全一致のみ**。前方一致・後方一致は行わない
    - `pyproject.toml` が存在するディレクトリからの相対パスで指定する
    - `allow-decorators` との OR 関係。どちらかに該当すれば違反としない

### 検出ロジック

1. `allow-files` にファイルパスが完全一致する場合、関数収集を行わず早期 `return`（ファイル単位の事前判定）
2. `ast.Module.body` を走査し、`ast.FunctionDef` および `ast.AsyncFunctionDef` ノードを収集する
3. 各ノードのデコレータリスト（`node.decorator_list`）を確認し、許可リストに含まれるデコレータが付いている場合はスキップする
4. 上記フィルタを通過したノードを違反として報告する

### 対象外のパターン

以下は違反として報告しません。

- **`allow-files` 登録ファイル**: `pyproject.toml` の `allow-files` にパスが完全一致するファイルはルール全体を除外
...
| ファイル単位の許可リスト | 該当ルールなし | `allow-files` で完全一致のファイルを除外 |
