...
class TestNoUnusedExportRuleCollectUsagesFromFile:
    """NoUnusedExportRule._collect_usages_from_file の直接テスト"""

    def test_collect_usages_from_file_正常系_戻り値がdictであること(self):
        # Arrange
        init_source = '__all__ = ["Foo"]\n'
        cli_source = "from paladin.check import Foo\n"
        source_files = make_source_files(
        ...
        rule = _rule(("paladin",))
        all_exports: dict[str, tuple[Path, dict[str, int]]] = rule._collect_all_exports(source_files)  # pyright: ignore[reportPrivateUsage]  # fmt: skip
        cli_file = list(source_files)[1]

        # Act
        result: dict[str, set[str]] = rule._collect_usages_from_file(cli_file, all_exports)  # pyright: ignore[reportPrivateUsage]

        # Assert
        assert isinstance(result, dict)

    def test_collect_usages_from_file_正常系_from_importの利用シンボルが収集されること(self):
        # Arrange
        ...
        # Assert
        assert result == {"paladin.check": {"Foo"}}

    def test_collect_usages_from_file_エッジケース_importなしの場合は空dictを返すこと(self):
        # Arrange
        init_source = '__all__ = ["Foo"]\n'
        other_source = "x = 1\n"
        source_files = make_source_files(
            (init_source, "src/paladin/check/__init__.py"),
            (other_source, "src/paladin/other.py"),
        )
        rule = _rule(("paladin",))
        all_exports: dict[str, tuple[Path, dict[str, int]]] = rule._collect_all_exports(source_files)  # pyright: ignore[reportPrivateUsage]  # fmt: skip
        other_file = list(source_files)[1]

        # Act
        result: dict[str, set[str]] = rule._collect_usages_from_file(other_file, all_exports)  # pyright: ignore[reportPrivateUsage]
