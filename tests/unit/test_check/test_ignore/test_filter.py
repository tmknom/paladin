...
class TestViolationFilter:
    ...
    def test_filter_正常系_ignore_allで該当ファイルの全違反が除外されること(self):
        ...
        violation = ViolationFactory.make(file_path, "rule-a")
    ...
    def test_filter_正常系_特定ルールignoreで該当ルールの違反のみ除外されること(self):
        # Arrange
        file_path = Path("example.py")
        v_a = ViolationFactory.make(file_path, "rule-a")
        v_b = ViolationFactory.make(file_path, "rule-b")
    ...
    def test_filter_正常系_ignore対象外のファイルの違反は保持されること(self):
        ...
        file_a = Path("a.py")
        file_b = Path("b.py")
        v_a = ViolationFactory.make(file_a, "rule-a")
        v_b = ViolationFactory.make(file_b, "rule-a")
        violations = Violations(items=(v_a, v_b))
        directive = FileIgnoreDirective(
    ...
    def test_filter_正常系_複数ファイルで各ファイルのディレクティブが正しく適用されること(self):
        ...
        file_a = Path("a.py")
        file_b = Path("b.py")
        v_a1 = ViolationFactory.make(file_a, "rule-a")
        v_a2 = ViolationFactory.make(file_a, "rule-b")
        v_b1 = ViolationFactory.make(file_b, "rule-a")
        v_b2 = ViolationFactory.make(file_b, "rule-b")
...
class TestViolationFilterLineIgnore:
    ...
    def test_filter_正常系_行単位ignore_allで該当行の全違反が除外されること(self):
        # Arrange
        file_path = Path("example.py")
        violation = ViolationFactory.make(file_path, "rule-a", line=5)
        violations = Violations(items=(violation,))
    ...
    def test_filter_正常系_行単位特定ルールignoreで該当ルールの違反のみ除外されること(self):
        # Arrange
        file_path = Path("example.py")
        v_a = ViolationFactory.make(file_path, "rule-a", line=5)
        v_b = ViolationFactory.make(file_path, "rule-b", line=5)
        violations = Violations(items=(v_a, v_b))
        line_directive = LineIgnoreDirective(
            file_path=file_path,
            target_line=5,
    ...
    def test_filter_正常系_行単位ignoreが異なる行の違反に影響しないこと(self):
        # Arrange
        file_path = Path("example.py")
        violation = ViolationFactory.make(file_path, "rule-a", line=3)
        violations = Violations(items=(violation,))
        line_directive = LineIgnoreDirective(
            file_path=file_path,
            target_line=5,
    ...
    def test_filter_正常系_ファイル単位と行単位のignoreが同時に適用されること(self):
        ...
        file_a = Path("a.py")
        file_b = Path("b.py")
        v_a = ViolationFactory.make(file_a, "rule-a", line=1)
        v_b_line3 = ViolationFactory.make(file_b, "rule-a", line=3)
        v_b_line5 = ViolationFactory.make(file_b, "rule-a", line=5)
    ...
    def test_filter_正常系_行単位ignoreが異なるファイルの違反に影響しないこと(self):
        ...
        file_a = Path("a.py")
        file_b = Path("b.py")
        violation = ViolationFactory.make(file_a, "rule-a", line=5)
        violations = Violations(items=(violation,))
        line_directive = LineIgnoreDirective(
            file_path=file_b,
            target_line=5,
...
class TestViolationFilterIgnoreRules:
    ...
    def test_filter_正常系_ignore_rulesとfile_directivesが同時に適用されること(self):
        ...
        file_b = Path("b.py")
        v_a = ViolationFactory.make(file_a, "rule-a")
        v_b = ViolationFactory.make(file_b, "rule-b")
        violations = Violations(items=(v_a, v_b))
        file_directive = FileIgnoreDirective(
            file_path=file_a, ignore_all=True, ignored_rules=frozenset()
        )
