...
from tests.unit.test_check.test_ignore.helper import SourceFileFactory
...
class TestIgnoreProcessor:
    ...
    def test_apply_正常系_ディレクティブなしで全違反が保持されること(self):
        ...
        violation = ViolationFactory.make(Path("example.py"), "rule-a")
    ...
    def test_apply_正常系_行単位ignoreで違反が除外されること(self):
        ...
        source_file = SourceFileFactory.make(source)
        source_files = SourceFiles(files=(source_file,))
        violation = ViolationFactory.make(Path("example.py"), "rule-a", line=2)
        violations = Violations(items=(violation,))
        processor = IgnoreProcessor()
    ...
    def test_apply_正常系_行末ignoreで違反が除外されること(self):
        ...
        source_file = SourceFileFactory.make(source)
        source_files = SourceFiles(files=(source_file,))
        violation = ViolationFactory.make(Path("example.py"), "rule-a", line=1)
        violations = Violations(items=(violation,))
        processor = IgnoreProcessor()
    ...
    def test_apply_正常系_直前コメントと行末コメントの累積適用で両ルールが除外されること(self):
        ...
        source_file = SourceFileFactory.make(source)
        source_files = SourceFiles(files=(source_file,))
        violation_a = ViolationFactory.make(Path("example.py"), "rule-a", line=2)
        violation_b = ViolationFactory.make(Path("example.py"), "rule-b", line=2)
