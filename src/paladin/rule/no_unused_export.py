...
class NoUnusedExportRule:
    ...
    def __init__(self) -> None:
        ...
        self._meta = RuleMeta(
            ...
            background="__init__.py の __all__ に定義したシンボルが別パッケージから利用されていない場合、不必要な後方互換義務が生じます。公開インタフェースを最小化することで内部実装の自由度とメンテナンス性が向上します。",
            steps=(
                "利用されていないシンボルを __all__ から削除する",
                "削除したシンボルへの参照が本当にないか確認する",
            ),
            detection_example='# 違反: 別パッケージから利用されていないシンボルが __all__ にある\n# myapp/services/__init__.py\n__all__ = [\n    "UserService",  # 別パッケージから利用されていない\n    "OrderService",\n]\n\n# 準拠: 利用されているシンボルのみを __all__ に列挙する\n__all__ = ["OrderService"]',
        )
